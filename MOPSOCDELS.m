function REP = MOPSOCDELS(params,MultiObj)
    ...
    for t=1:MaxDT 
        ...
        for i=1:column   
            if rand<sigm
                ...
                mp(mi,1:d) = ELS(rp(i,1:d),gauss,xmin,xmax);
                mp(mi,1+d:d+d) = rp(i,1+d:d+d);
                for j=1:d 
                    if mp(mi,j)>xmax(j) 
                        mp(mi,j)=xmax(j);
                    elseif mp(mi,j)<xmin(j) 
                        mp(mi,j)=xmin(j);
                    end 
                end 
                mp(mi,2*d+1:2*d+k)=fun(mp(mi,1:d));
                mi = mi+1;
            end
        end
        np = [np;mp];
...
function xnew=ELS(x,gauss,VarMin,VarMax)
    nVar=numel(x);
    j=randi([1 nVar]);
    dx=gauss*(VarMax(j)-VarMin(j));
    xnew=x;
    xnew(j)=x(j)+dx;
    if xnew(j)<VarMin(j)
        xnew(j)=VarMin(j);
